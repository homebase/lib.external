	...
	function save($refresh = false) {
		...
		if ($this->_id) {//if exists
			if (!empty($this->_operations)) {
				#mike $bool = $this->_collection->update(array( "_id" => $this->_id ), $this->_operations, array(  "upsert" => false, "multiple" => false, "safe" => true ));
				$bool = $this->_collection->update(array( "_id" => $this->_id ), $this->_operations, array(  "upsert" => false, "multiple" => false, "w" => 1 ));
				if ($refresh) {
					$bool = $this->refresh();
				}
			}
