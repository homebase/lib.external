...
abstract class Kount_Ris_Request {
 ...
 MONTH  * @param string $mode RIS mode
  ...
  public function setCardPayment ($cardNumber) {
    ...
    // Only necessary for certain 3rd party services.
    if ($this->data['MODE'] != 'U' && $this->data['MODE'] != 'P' &&
        $this->data['PTYP'] == 'CARD' && !empty($cardNumber)) {
      $this->data['EPTOK'] = $this->encrypt($cardNumber);
    }
  ...
  /**
   * RSA encrypt a plaintext string. Used for encrypting credit card numbers.
   *
   * @param string $plainText Plain text to encrypt
   * @return base64 encoded string of the encrypted card number
   */
  protected function encrypt ($plainText) {
    if (function_exists("openssl_public_encrypt")) {
      $encrypted = "";
      $publicKey = file_get_contents(RSA_PUBLIC_KEY);
      openssl_public_encrypt($plainText, $encrypted, $publicKey);
      return base64_encode($encrypted);
    }
    // openssl not installed. Fail silently because most merchant won't
    // need this feature.
    return "";
  }
