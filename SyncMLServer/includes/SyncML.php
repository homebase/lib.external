 ...
 *
 * $Horde: framework/SyncML/SyncML.php,v 1.72 2008/04/08 23:46:32 jan Exp $
 *
...
require_once MY_PATH.'Horde/String.php';
require_once MY_PATH.'SyncML/State.php';
require_once MY_PATH.'SyncML/Command.php';
require_once MY_PATH.'SyncML/Command/SyncHdr.php';
require_once MY_PATH.'SyncML/Sync.php';
require_once MY_PATH.'SyncML/XMLOutput.php';

/*Humanbook contacts interface*/
//require_once MY_PATH.'Ab_SyncMLApi.php';


/* PEAR_LOG_* constants */
//require_once MY_PATH.'Log.php';
...
class SyncML_ContentHandler {
    ...
    {
#Mike
        $isWBXML = (strpos($contentType, 'application/vnd.syncml+wbxml') === 0);
...
#Mike
		if ($isWBXML) {
            require_once MY_PATH.'XML/WBXML/Decoder.php';
			$_decoder = new XML_WBXML_Decoder();
	        $GLOBALS['backend']->logFile(SYNCML_LOGFILE_CLIENTMESSAGE, $_decoder->decodeToString($request), false);
		}
        ...
        if (!$isWBXML) {
             ...
             * output. */
            require_once MY_PATH.'XML/WBXML/ContentHandler.php';
            ...
            /* WBXML code. */
            require_once MY_PATH.'XML/WBXML/Decoder.php';
            require_once MY_PATH.'XML/WBXML/Encoder.php';
...
#Mike
		if ($isWBXML) {
			$_decoder = new XML_WBXML_Decoder();
	        $GLOBALS['backend']->logFile(SYNCML_LOGFILE_SERVERMESSAGE, $_decoder->decodeToString($output), false);
		}
    ...
    function handleHeader(&$hdr)
    {
        ...
        if (!$state->authenticated) {
            ...
            if ($auth === true) {
                ...
                $state->user = $hdr->user;
                $GLOBALS['backend']->setUser($hdr->user);
            } else {
                if (!$hdr->credData) {
        ...
        $this->_xmlWriter->outputHeader(null/*there was no parameter. Mike added NULL to avoid warnings*/);
