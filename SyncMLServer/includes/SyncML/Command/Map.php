...
class SyncML_Command_Map extends SyncML_Command {
    ...
    /**
     * Hash of cuid => suid maps to process.
     *
     * @var boolean
     */
    var $_mapItems = array();
...
//O_Log::logMessage("PROCESSING MapItem: ".print_r($this->_stack, 1).", $this->_mapItem, $this->_mapTarget, $this->_mapSource" , __LINE__, PEAR_LOG_DEBUG);
    ...
    function endElement($uri, $element)
    {
        ...
        case 2:
        	if ($element == 'MapItem') {
        		$this->_mapItems[$this->_mapSource] =  $this->_mapTarget;
        	}        	
        	break; 
        case 3:
            if ($element == 'LocURI') {
    ...
    function handleCommand($debug = false)
    {
        if (!$debug && $this->_mapItem) {
            ...
            if (!$state->authenticated) {
            	...
            	foreach ($this->_mapItems as $cuid => $suid) {
