...
class SyncML_Backend {
     ...
     */
    static function factory($driver, $params = null)
    {
        if (empty($driver) || ($driver == 'none')) {
            return false;
        }
        if (basename($driver) == $driver) {
            include_once MY_PATH.'SyncML/Backend/' . $driver . '.php';
        } else {
            include_once MY_PATH.$driver . '.php';
        }
     ...
     *                           - PEAR_LOG_DEBUG
     */
    function logMessage($message, $file = __FILE__, $line = __LINE__, $priority = PEAR_LOG_INFO)
    {
        if ($priority > $this->_logLevel)  {
            var_dump($priority); var_dump($this->_logLevel);
            return;
        }
    ...
    function logFile($type, $content, $wbxml = false, $sessionClose = false)
    {
        ...
        if ($type === SYNCML_LOGFILE_CLIENTMESSAGE ||
            ...
            if (file_exists($this->_debugDir . '/packetnum.txt')) {
            	$packetNum = @intval(file_get_contents($this->_debugDir . '/packetnum.txt'));
            }
            if (empty($packetNum)) {
                $packetNum = 10;
            }
    ...
    function close()
    {
        if (!empty($this->_debugDir)) {
            $f = @fopen($this->_debugDir . '/common.log', 'a+');
            if ($f) {
